...
export default function ImportarScreen() {
  ...
  const { url, text, image, video } = useLocalSearchParams<{ url?: string; text?: string; image?: string; video?: string }>();
  ...
  useEffect(() => {
    ...
    const shared: ImportInput | null = video
      ? { videoUri: video }
      : image
        ? { imageUri: image }
        : url
